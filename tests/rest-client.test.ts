...
test('RestClient attaches API key header for protected endpoints', async () => {
...
test('RestClient throws when protected endpoint is called without API key', async () => {
...
test('RestClient surfaces HTTP errors with parsed body', async () => {
  globalThis.fetch = async () => {
...
test('RestClient expands attestation records', async () => {
  globalThis.fetch = async () => {
  ...
  assert.equal(attestation.tags_json?.contract_name, 'Uniswap Router');
...
test('RestClient deduplicates identical in-flight requests', async () => {
  let calls = 0;
  globalThis.fetch = async () => {
    calls += 1;
...
test('RestClient caches responses when enabled', async () => {
  let calls = 0;
  globalThis.fetch = async () => {
    calls += 1;
    return new Response(JSON.stringify({ address: '0xabc', count: 0, labels: [] }), {
